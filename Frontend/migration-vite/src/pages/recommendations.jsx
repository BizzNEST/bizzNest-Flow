...
const Recommendations = () => {
  ...
  const groupAndSortRecommendations = useCallback((recommendations, asc) => {
    ...
    recommendations.forEach((intern) => {
      ...
      intern.calculations.forEach((calc) => {
        ...
        if (
          !groups[toolID].some(
            (existing) => existing.internID === numericInternID
          )
        ) {
          groups[toolID].push({
            ...
            name:
              ...
              `Intern ${numericInternID}`,
            percent:
              typeof calc.percentIncrease === "number"
                ? calc.percentIncrease
                : 0,
    ...
    Object.values(groups).forEach((arr) =>
      arr.sort((a, b) => {
        if (asc) {
          // Leadership sort: Eligible first, then by lowest % gain (already skilled)
          ...
          // Learning sort: Highest % gain first
  ...
  useEffect(() => {
    ...
    console.log("Effect triggered: Fetching data for", {
      projectID,
      departmentID,
    });
    ...
    (async () => {
      try {
        ...
        try {
          ...
          throw new Error(
            `Server returned non-JSON response with status: ${res.status} ${res.statusText}`
          );
        }

        if (!isMounted) {
          console.log(
            "Component unmounted during fetch, aborting state update."
          );
          return;
        }
        ...
        if (!res.ok) {
          console.error(
            `Recommendations fetch failed (Status: ${res.status}):`,
            payload
          );
          throw new Error(
            payload?.message ||
        ...
        if (
          ...
          throw new Error(
            "Received invalid or empty project data structure from server."
          );
        }
        // CRITICAL CHECK: Ensure assignedInterns is an array (even if empty)
        if (
          !payload.assignedInterns ||
          !Array.isArray(payload.assignedInterns)
        ) {
          console.error(
            "Payload validation failed: 'assignedInterns' is missing or not an array.",
            payload
          );
          throw new Error(
            "Received incomplete data structure from server (missing or invalid 'assignedInterns' array)."
          );
        }
        // Also validate skillDifferences for safety
        if (
          !payload.skillDifferences ||
          !Array.isArray(payload.skillDifferences)
        ) {
          console.warn(
            "Payload warning: 'skillDifferences' is missing or not an array. Recommendations might be empty.",
            payload
          );
        ...
        projectInfo.tools = Array.isArray(projectInfo.tools)
          ? projectInfo.tools
        ...
        console.log(
          `Stored ${payload.skillDifferences.length} skill difference entries.`
        );
        ...
        console.log(
          `Processing ${payload.assignedInterns.length} assigned interns for initial state...`
        );
        payload.assignedInterns.forEach((assignment) => {
          const internID = Number(assignment.InternID); // Ensure numeric ID
          if (isNaN(internID)) {
            console.warn(
              "Skipping assigned intern with invalid ID:",
              assignment.InternID
            );
            return;
          }
          ...
          if (role === "Leader") {
            ...
            // Default to Intern if role is not explicitly "Leader"
        ...
        const groupedData = groupAndSortRecommendations(
          payload.skillDifferences,
          isAscending
        );
    ...
    return () => {
      console.log(
        "Recommendations component unmounting or dependencies changed."
      );
      isMounted = false;
    };
    ...
    location.search,
    getQueryParams,
    groupAndSortRecommendations,
    isAscending,
  ]); // Add isAscending here so sorting reruns when toggled
  ...
  const submitChanges = useCallback(async () => {
    ...
    const desiredAssignments = [
      ...Array.from(selectedInterns).map((id) => ({
        internID: id,
        role: "Intern",
        projectID: numericProjectID,
      })),
      ...Array.from(selectedLeaders).map((id) => ({
        internID: id,
        role: "Leader",
        projectID: numericProjectID,
      })),
    ...
    const currentAssignmentsMap = new Map(
      desiredAssignments.map((a) => [a.internID, a.role])
    );
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.error(
          `Failed to update assignments (Status: ${res.status}):`,
          responseData
        );
        throw new Error(
  ...
  const grad = (p) => {
    ...
    return p >= 5
      ...
      : p >= 0
        ? "linear-gradient(to bottom,rgb(206,91,42),rgb(251,179,45))"
    ...
    return (
      <div className={styles.errorContainer}>
        ...
        <p className={styles.errorText}>
          Could not load project data. Please try again.
        </p>
        <button onClick={() => window.location.reload()}>Retry</button>
      </div>
    );
  ...
  const validTools = projectTools.filter(
    (t) => typeof t.difficulty === "number"
  );
  const avgDifficulty =
    validTools.length > 0
      ? (
          validTools.reduce((sum, t) => sum + t.difficulty, 0) /
          validTools.length
        ).toFixed(1)
      : "N/A";
  ...
  return (
    <div className={styles.recommendationsContainer}>
      ...
      <div className={styles.projectStats}>
        <div className={styles.projectTitle}>
          <div className={styles.titleBox}>
            {projectData.projectTitle || "Unnamed Project"}
          </div>
        </div>
      ...
      <div className={styles.suggestionsContainer}>
        {projectTools.length === 0 ? (
          <p className={styles.noRecommendationsOverall}>
            No tools assigned to this project, cannot show recommendations.
          </p>
        ) : (
          projectTools.map((tool) => (
            <div
              key={`rec-tool-${tool.toolID}`}
              className={`tool-row-${tool.toolID}`}
            >
              <h3 className={styles.toolHeader}>
              ...
              <div className={styles.tabletRows}>
                <div className={styles.rowTablets}>
                  ...
                  {!grouped?.[tool.toolID] ||
                  grouped[tool.toolID]?.length === 0 ? (
                    <p className={styles.noRecommendations}>
                      No interns recommended for this skill.
                    </p>
                  ) : (
                    ...
                    grouped[tool.toolID].map((internRec, idx) => {
                      ...
                      const isSelectedAsIntern = selectedInterns.has(
                        internRec.internID
                      );
                      const isSelectedAsLeader = selectedLeaders.has(
                        internRec.internID
                      );
